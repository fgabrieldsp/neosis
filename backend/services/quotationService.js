...
function generateQuote(projectBlocks, overrides = {}, profileMapping = {}) {
    const params = {
        ...
        preco_osmocolor_m2: overrides?.service_costs?.preco_osmocolor_m2?.value ?? baseConfig.service_costs.preco_osmocolor_m2.value,
    ...
    const detailedBom = [];
    const costSummary = {
        materiais: { metal: 0, madeira: 0, cobertura: 0, acessorios: 0, tinta: 0 },
        servicos_detalhados: { fundacao: 0, mao_de_obra_pintura: 0, usinagem: 0, galvanizacao: 0, osmocolor: 0, montagem: 0, logistica: 0 },
        totais_agrupados: { total_materiais: 0, total_servicos: 0 }
    };
    let physicalTotals = { total_weight_metal: 0, total_surface_area_metal: 0, total_surface_area_madeira: 0 };
    ...
    finalBomMap.forEach((data, partId) => {
        ...
        if (profile) {
            totalWeight = (profile.weight_kg_per_meter || 0) * (component.length_m || 0) * qty;
            totalSurfaceArea = (profile.surface_area_m2_per_meter || 0) * (component.length_m || 0) * qty;
        } else {
            totalWeight = ((component.weight_kg_per_meter || 0) * (component.length_m || 0) + (component.weight_kg_per_unit || 0)) * qty;
            totalSurfaceArea = ((component.surface_area_m2_per_meter || 0) * (component.length_m || 0) + (component.surface_area_m2_per_unit || 0)) * qty;
        }
        if (component.manufacturing_recipe && profileId) {
            const rawMaterialKey = `${component.manufacturing_recipe.raw_material_key_prefix}${profileId}`;
            const rawMaterial = catalog.materias_primas[rawMaterialKey];
            if (rawMaterial) {
                const existingNeed = manufacturingNeeds.get(rawMaterialKey) || { total_length: 0 };
                existingNeed.total_length += (component.length_m || 0) * qty;
                manufacturingNeeds.set(rawMaterialKey, existingNeed);
            }
        }
        switch (component.category) {
            ...
            case 'servicos':
                ...
                costSummary.servicos_detalhados.fundacao += itemCost;
                break;
        }
    ...
    costSummary.servicos_detalhados.usinagem = physicalTotals.total_weight_metal * params.custo_usinagem_kg;
    ...
    costSummary.servicos_detalhados.osmocolor = physicalTotals.total_surface_area_madeira * params.preco_osmocolor_m2;
    ...
    if (params.logistica_ativo) costSummary.servicos_detalhados.logistica = physicalTotals.total_weight_metal * params.logistica_custo_base_kg * params.logistica_coeficiente;
    ...
    const engineering_report = { total_weight_steel_kg: physicalTotals.total_weight_metal, total_paint_area_m2: physicalTotals.total_surface_area_metal, raw_materials: Array.from(manufacturingNeeds.entries()).map(([rawId, needs]) => { const rawItem = catalog.materias_primas[rawId]; if (!rawItem) return null; const units_needed = Math.ceil(needs.total_length / rawItem.length_m); return { name: rawItem.name, profile_id: rawItem.profile_id, total_linear_meters: needs.total_length, units_to_buy: units_needed, unit_length_m: rawItem.length_m }; }).filter(item => item !== null), paint_report: { total_paint_area_m2: physicalTotals.total_surface_area_metal, primer_liters: primerLitersNeeded, primer_cost: primerCost, pu_liters: puLitersNeeded, pu_cost: puCost, total_paint_cost: primerCost + puCost } };
