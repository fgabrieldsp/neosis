// VERSÃO FINAL CORRIGIDA - app.js com renderização de blocos robusta
...
async function populateInitialData() {
    ...
    } catch (error) { console.error("Erro fatal na inicialização:", error); alert(`Erro na inicialização: ${error.message}`); }
}
...
async function calculateQuote(overrides = null) {
    ...
    try {
        ...
        if (overrides && Object.keys(overrides).length > 0) requestBody.overrides = overrides;
        
        const response = await fetch(`${API_BASE_URL}/api/quote`, { method: 'POST', headers: { 'Content-Type': 'application/json' }, body: JSON.stringify(requestBody) });
        if (!response.ok) {
        ...
        renderUI();
    } catch (error) { console.error("Erro ao calcular orçamento:", error); alert(`Erro no cálculo: ${error.message}`); }
}

function handleAddBlock() {
    const vagas = parseInt(ui.vagasInput.value, 10);
    if (isNaN(vagas) || vagas <= 0) { alert("Por favor, insira um número de vagas válido."); return; }
    const estruturaId = ui.estruturaSelect.value;
    if (!estruturaId) { alert("Por favor, selecione um modelo estrutural."); return; }
...
// ### FUNÇÃO renderProjectBlocks CORRIGIDA E ROBUSTA ###
function renderProjectBlocks() {
    if (!ui.blockList) return;
    
    ui.blockList.innerHTML = ''; // Limpa a lista completamente

    if (state.projectBlocks.length === 0) {
        // Se estiver vazia, adiciona o placeholder
        ui.blockList.innerHTML = '<li class="list-group-item text-muted" id="placeholderLista">Nenhum bloco adicionado.</li>';
    } else {
        // Se houver blocos, cria os LIs para cada um e os junta
        const blocksHtml = state.projectBlocks.map((block, index) => `
        ...
        `).join('');
        ui.blockList.innerHTML = blocksHtml;
    }
}
...
function renderEngineeringReport() { const report = state.currentQuote?.engineering_report; if (!report) { ui.engineeringContainer.innerHTML = '<p class="text-muted">Nenhum dado de engenharia para exibir.</p>'; return; } const rawMaterialsRows = report.raw_materials.map(item => `<tr><td>${item.name}</td><td class="text-end">${item.total_linear_meters.toFixed(2)} m</td><td class="text-end">${item.units_to_buy} un</td></tr>`).join(''); const paintReportHtml = report.paint_report && report.paint_report.total_paint_area_m2 > 0 ? `<div class="card mt-4"><div class="card-header"><i class="bi bi-palette-fill"></i> Relatório de Insumos de Pintura</div><div class="card-body"><ul class="list-group list-group-flush"><li class="list-group-item d-flex justify-content-between align-items-center"><div>Consumo de Primer <small class="text-muted d-block">com margem de segurança</small></div><strong>${report.paint_report.primer_liters.toFixed(2)} L</strong></li><li class="list-group-item d-flex justify-content-between align-items-center">Custo do Primer<strong>${formatCurrency(report.paint_report.primer_cost)}</strong></li><li class="list-group-item d-flex justify-content-between align-items-center"><div>Consumo de Tinta PU <small class="text-muted d-block">com margem de segurança</small></div><strong>${report.paint_report.pu_liters.toFixed(2)} L</strong></li><li class="list-group-item d-flex justify-content-between align-items-center">Custo da Tinta PU<strong>${formatCurrency(report.paint_report.pu_cost)}</strong></li><li class="list-group-item d-flex justify-content-between align-items-center table-light"><strong>Custo Total (Insumos de Tinta)</strong><strong>${formatCurrency(report.paint_report.total_paint_cost)}</strong></li></ul></div></div>` : ''; ui.engineeringContainer.innerHTML = `<div class="row g-3"><div class="col-lg-6"><div class="card h-100"><div class="card-header"><i class="bi bi-rulers"></i> Indicadores Físicos</div><div class="card-body"><ul class="list-group list-group-flush"><li class="list-group-item d-flex justify-content-between"><span>Peso Total do Aço</span><strong>${report.total_weight_steel_kg.toFixed(2)} kg</strong></li><li class="list-group-item d-flex justify-content-between"><span>Área de Pintura (Aço)</span><strong>${report.total_paint_area_m2.toFixed(2)} m²</strong></li></ul></div></div></div><div class="col-lg-6"><div class="card h-100"><div class="card-header"><i class="bi bi-box-seam"></i> Lista de Insumos para Compra</div><div class="card-body"><table class="table table-sm table-borderless"><thead><tr><th>Matéria-Prima</th><th class="text-end">Metros Lineares</th><th class="text-end">Unidades de Compra</th></tr></thead><tbody>${rawMaterialsRows}</tbody></table></div></div></div></div>${paintReportHtml}`; }
...
document.addEventListener('DOMContentLoaded', () => {
    Object.values(ui).forEach(element => { if (!element) console.warn('Um elemento da UI não foi encontrado no DOM.'); });
